...
def singleTest(numRow, numCol):
    ...
    # print('rlnc')
    enc1 = bc.RLNCEncoder2(numRow, numCol, sysPhase=False, dtype=dtype)
    dec1 = bc.Decoder()
    decodable = False  # a flag indicating whether the k tasks can be decoded
    counter1 = 0
    while not decodable:
        coeff = enc1.getCoeff()
        useful, decodable = dec1.receive(coeff)
        counter1 += 1
    # counter1 = k
    # print('lt')
...
# redun['kList'] = kTest
# with open('20171129_float2.pickle', 'wb') as handle:
#     pickle.dump(redun, handle)

assert False
kTest = range(100, 1001, 100)
with open('20171129_float.pickle', 'rb') as handle:
    redun1 = pickle.load(handle)
with open('20171129_float2.pickle', 'rb') as handle:
